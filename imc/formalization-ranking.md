...
Note: IMC had 4 problems/day in 2020-2022 (8/year) and 5 problems/day in 2023-2025 (10/year), giving 54 problems in total across these six years (not 60).
...
| 7 | IMC 2024 P7 | statement formalized | invertible A+B=I, polynomial identity, det(AB) values | Low | Low | Reduces to polynomial equation in C = AB; elementary. |
...
| 9 | IMC 2022 P2 | statement formalized | A+A^k=A^T with real eigenvalues, find all A | Low | Low-Med | Minimal polynomial argument; A=0. |
| 10 | IMC 2024 P2 | statement formalized | limit of Riemann-sum type expression involving log | Low | Low | Convert to Riemann sum for integral of x log x. |
...
| 27 | IMC 2023 P3 | not started | P(x,y)P(z,t) = P(xz-yt, xt+yz) classify polynomials | Low | Med | Complex factorization (x+iy)^n(x-iy)^m; real-coef constraint. |
| 28 | IMC 2025 P4 | statement formalized | floor identity for b^{a-1} (b^a+x)^{1/a} | Low-Med | Med | Bernoulli inequality + case analysis. |
| 29 | IMC 2021 P8 | not started | unit vectors with 3-at-a-time orthogonality condition | Low-Med | Med | Projector trace identities; algebraic inequalities. |
...
| 31 | IMC 2021 P6 | not started | no injection GL_2(F_p) -> S_p | Med | Med | Element-of-order-2p + structure of S_p. |
...
| 39 | IMC 2023 P8 | not started | tree Wiener index times harmonic index bound | Med | Med | Cauchy-Schwarz; needs graph theory library (tree, distance). |
| 40 | IMC 2022 P5 | not started | count monochromatic triangles on K_{43} | Med | Med-High | Double counting 'cherries'; specific 43-vertex problem. |
...
| 43 | IMC 2024 P4 | not started | subgroup gen. by n-grams independent of sequence | Med-High | High | Pigeonhole + non-periodicity + induction on group. |
...
| 48 | IMC 2022 P4 | not started | chromatic number of triple-graph, log log n bounds | Med-High | Very High | Heavy graph coloring framework; iterated chromatic number. |
