...
| 20 | IMC 2001 P1 | not started | - | n x n grid has permutation-transversal with prescribed sum | Low | Low | Any permutation gives the sum; direct evaluation. |
...
| 34 | IMC 2017 P2 | not started | - | Lipschitz derivative and f>0 implies (f')^2 < 2Lf | Low | Low-Med | Classical: use Taylor expansion of f at nearby point. |
| 35 | IMC 2009 P8 | not started | - | A^2 B + B A^2 = 2 A B A implies AB - BA nilpotent | Low | Low-Med | Expand powers of (AB-BA); use given identity to reduce. |
| 36 | IMC 2017 P6 | not started | - | integral of f(nx) over [0,1] tends to lim f | Low | Low-Med | Substitute u=nx and split at a large threshold. |
...
| 79 | IMC 2005 P5 | not started | - | \|f''+2xf'+(x^2+1)f\| <= 1 implies f(x) -> 0 at infinity | Low-Med | Med | Integrating factor e^{x^2/2}; L'Hopital on growth ratio. |
...
| 91 | IMC 2007 P3 | not started | - | quadratic forms representable as det(sum x_i A_i) over 2x2 | Low-Med | Med | Discriminant and trace-zero basis argument. |
| 92 | IMC 2006 P5 | not started | - | power-sum triple comparison of a^3 + b^3 + c^3 vs d^3 + e^3 | Low | Med | Bounding via power-mean inequalities. |
...
| 97 | IMC 2006 P4 | not started | - | rational function integer-valued on infinitely many integers is polynomial | Low | Med | Degree of denominator bounded; polynomial division. |
| 98 | IMC 2000 P7 | not started | - | unit cube dissectible into n smaller cubes for all n >= N | Low | Med | Split one cube into 8; base-case induction on n. |
...
| 112 | IMC 2001 P6 | not started | - | ratio-limit f'/g' + a f/g tends to A + 1 | Med | Med | Generalized L'Hopital with additive correction. |
...
| 130 | IMC 2003 P11 | not started | - | R->R function uniformly bounded by g(x) + g(y); classify g | Low-Med | Med | Inf-over-translate; subadditive decomposition. |
...
| 157 | IMC 2009 P9 | not started | - | permutation group over F_p generated by two specific polynomials | Med | Med-High | Finite permutation group classification; degree analysis. |
| 158 | IMC 2005 P12 | not started | - | (p + q sqrt 7) Pell-style fixed point of SL_2(Z) Mobius | Med | Med-High | Pell equation + SL_2(Z) orbit analysis. |
| 159 | IMC 2008 P6 | not started | - | permutation displacement D(sigma) distribution parity for large d | Med | Med-High | Generating-function parity count; asymptotic. |
| 160 | IMC 2001 P4 | not started | - | polynomial with coeffs in {-1,0,1}, divisibility forces q-th roots as roots | Med | Med-High | Cyclotomic divisor analysis; sparse coefficient bound. |
...
| 165 | IMC 2003 P6 | not started | - | log-concavity of coefficients for stable (all real-root) polynomials | Low | Med-High | Newton's inequalities on elementary symmetric polys. |
...
| 192 | IMC 2018 P9 | not started | - | classify monic complex P,Q with P \| Q^2+1 and Q \| P^2+1 | Low | Med-High | Degree comparison; classify via iteration identity. |
